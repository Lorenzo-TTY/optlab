...
- 本地优化项目的主要持久化源应是后端 SQLite 项目快照；浏览器 `localStorage` 只能作为后端不可用或离线场景的降级缓存，避免服务重启、端口变化或访问入口变化导致用户误以为项目丢失。
...
- 用户反馈：保存的项目在关闭服务后无法再打开。根因是项目只写入当前浏览器 origin 的 `localStorage`，服务重启、端口变化或 `localhost`/`127.0.0.1` 访问入口变化会让前端看不到之前的项目缓存。
- 已改为后端 SQLite 保存 `/api/projects` 项目快照，并让前端启动时先显示浏览器缓存、再从本地后端项目快照恢复；浏览器 `localStorage` 保留为后端不可用时的降级缓存。
- 已新增后端项目快照重启恢复测试和前端“浏览器缓存为空但后端有项目快照时恢复项目”的测试；验证通过：`frontend` 下 `npm run test` 通过 15 个测试，`frontend` 下 `npm run build` 通过；`backend\.venv\Scripts\python.exe -m pytest backend\tests -q` 通过 45 个测试；`backend\.venv\Scripts\python.exe -m pytest notebooks -q` 通过 3 个测试；`git diff --check` 无空白错误，仅提示 Windows 行尾转换。
- 浏览器服务重启复测通过：在 `http://127.0.0.1:5173/` 创建并保存 `Restart persisted ...` 项目和 1 条 observation，确认 `/api/projects` 写入后停止并重启 8000 后端，清空浏览器 `localStorage` 再打开前端，界面从后端项目快照恢复项目名、`manual_000001` submitted 行和 `1 saved`；浏览器 console 仅有 React DevTools info。

...
- 支持本地优化项目管理：在侧边栏新建、重命名、保存和切换项目；项目快照写入后端 SQLite，并同步到浏览器缓存作为降级恢复路径。
...
- 后端 SQLite 项目快照：作为主要恢复来源，包含问题定义、草稿行、advisor 建议行、已保存 observations、advisor 状态和更新时间；关闭服务后重新启动本地后端，再打开前端时会从这里恢复项目列表和最后激活项目。
- 浏览器 `localStorage` 缓存：作为后端暂不可用、网络请求失败或离线使用时的降级恢复路径。

Result Summary、Pareto preview、best-per-objective 和 coverage min 会从已保存 observations 实时重算，不作为独立权威数据保存。服务完全关闭时网页本身仍需重新启动前端服务才能访问；重新启动后，项目数据不依赖原来的浏览器 origin。
...
参数定义表支持从 Excel、WPS、Google Sheets 等表格工具直接复制矩形区域后粘贴。列顺序与界面一致：

```text
Name    Type    Lower   Upper   Scale
temperature float 10    100     linear
pressure    float 1     5       log
```

从任意单元格开始粘贴时，数据会从该单元格向右、向下填充；粘贴的行数超过当前参数行数时，界面会自动扩展参数维度，最多 30 维。
...
```text
Name    Direction   Unit    Threshold
cost    min         USD
quality max         pts     0.8
```

超过当前目标行数的粘贴会自动扩展目标维度，最多 6 维。
...
也可以从 Excel、WPS 或 Google Sheets 复制多行多列数据，直接粘贴到任意参数或目标单元格。粘贴会按当前单元格位置向右、向下填充；如果粘贴的行数超过当前可编辑行数，界面会自动追加手工行。
...
- 从 Excel/表格复制参数定义、目标定义和 ask/tell 数据行后按行列批量粘贴。
- 维度优先输入。
- 参数/目标定义表自动生成。
- advisor 请求。
- 候选建议表渲染。
...
- 交互式项目持久化使用后端 SQLite 项目快照，并保留浏览器 `localStorage` 作为降级缓存；跨设备同步和导入/导出属于后续增强。
